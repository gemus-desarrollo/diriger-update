...
$acc= null;
$acc_planwork= !empty($_SESSION['acc_planproject']) ? $_SESSION['acc_planproject'] : 0;

$id_calendar= !empty($_GET['id_calendar']) ? $_GET['id_calendar'] : $_SESSION['id_calendar'];
...
$url_page= "../html/gantt_user.php?signal=$signal&action=$action&menu=tablero&id_proceso=$id_proceso";
$url_page.= "&year=$year&month=$month&day=$day&id_proyecto=$id_proyecto&id_programa=$id_programa&id_calendar=$id_calendar";
...
<head>
    ...
    <title>GANTT - INDIVIDUAL</title>
    <?php require_once "inc/_proyecto_top_head.inc.php"; ?>

    <link rel="stylesheet" type="text/css" href="../libs/jsGantt-1.7.5.4/jsgantt-1.8.0.css?version=" charset="utf-8">
...
<body>
    <?php require_once "inc/_tarea_tablero_top.inc.php";?>  
        ...
        <div class="gantt" id="GanttChartDIV">
            ...
            <script type="text/javascript" charset="utf-8">
                ...
                if (g.getDivId() != null) {
                    ...
                    $obj_task->SetDay($day);
                    $obj_task->SetIdProyecto(null);
                    $obj_task->SetIdProceso(null);
                    $obj_task->SetIdResponsable($id_calendar);

                    $result= $obj_task->listar(false, null, null, true);
                    ...
                    while ($row= $clink->fetch_array($result)) {
                        ...
                        $pComp= !empty($array[0]['valor']) ? $array[0]['valor'] : "''";
                    ...
                    //	$pCaption;
                    ?>
                    g.AddTaskItem(new JSGantt.TaskItem(<?=$row['_id']?>, '<?=$pName?>', '<?=$pStart?>', '<?=$pEnd?>', '<?=$pClass?>', '<?=$pLink?>',  <?=$pMile?>, '<?=$pRes?>', <?=$pComp?>, <?=$pGroup?>, <?=$pParent?>, <?=$pOpen?>, '<?=$pDepend?>', '', 'Observacion aqui', g));
