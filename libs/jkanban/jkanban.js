            ...
            this.jKanban = function() {
                ...
                this.addBoards = function(boards, isInit) {
                    ...
                    for (var boardkey in boards) {
                        ...
                        var hyperDelete = document.createElement('a')
                        var iDelete = document.createElement('i');
                        iDelete.classList.add('fa');
                        iDelete.classList.add('fa-trash');
                        hyperDelete.classList.add('board-icon');
                        hyperDelete.appendChild(iDelete);
                        headerBoard.appendChild(hyperDelete);
                        // mustelier
                        __onDeleteClickHandler(hyperDelete, board.id);
                        __onColumnClickHandler(headerBoard, board.id);
                        ...
                        if (addButton) {
                            ...
                            btn.appendChild(t)
                                //var buttonHtml = '<button class="kanban-title-button btn btn-default btn-sm">'+buttonContent+'</button>'
                            if (buttonFooter) {
                ...
                // mustelier 
                function __onDeleteClickHandler(nodeItem, boardId) {
                    nodeItem.addEventListener('click', function(e) {
                        e.preventDefault()
                        self.options.deleteBoard(this, boardId)
                            // if (typeof(this.clickfn) === 'function')
                            //     this.clickfn(this);
                    })
                }

                function __onColumnClickHandler(nodeItem, boardId) {
                    nodeItem.addEventListener('click', function(e) {
                        e.preventDefault()
                        self.options.selectBoard(this, boardId)
                            // if (typeof(this.clickfn) === 'function')
                            //     this.clickfn(this);
                    })
                }
                ...
                function __updateBoardsOrder() {
                    ...
                    // mustelier
                    for (var i = 0; i < self.container.childNodes.length; i++) {
                        id = self.container.childNodes[i].getAttribute('data-id');
                        number = self.container.childNodes[i].getAttribute('data-order')
                        kanban_columns_order[i] = [id, number]
                            // console.log('data-id=' + id, '   date-order=' + number);
                    }
                }
